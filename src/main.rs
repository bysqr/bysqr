...
fn insert_pay_text(svg: &mut Element, container_width: f64, container_height: f64, color: &str) {
    /// Text dimensions: 69x29
...
fn insert_by_square_text(svg: &mut Element, container_width: f64, container_height: f64, color: &str) {
    /// Text dimensions: 189x39
...
fn insert_pay_icon(svg: &mut Element, container_width: f64, container_height: f64, color: &str) {
    let icon_size = 98.0;
...
fn create_empty_svg(container_width: f64, container_height: f64) -> Element {
...
fn main() {
    ...
    let mut content_to_show: Option<String> = None;
        ...
        Some(Commands::Encode { xml, show }) => {
            if let Some(path) = xml {
                ...
                if *show {
                    ...
                    let container_width = 512.0;
                    let container_height = 600.0;

                    let outline_color = "#6fa4d7";
                    let icon_color = "#6fa4d7";
                    let by_square_text_color = "#b2b4b9";
                    let pay_text_color = "#6fa4d7";

                    /// QR code should be sized exactly like outline width with some padding around.
                    ...
                    let mut final_svg = create_empty_svg(container_width, container_height);
                    insert_qr_content(&mut final_svg, &svg_image, container_width);
                    insert_outline(&mut final_svg, &outline_color);
                    insert_pay_icon(&mut final_svg, container_width, container_height, &icon_color);
                    insert_by_square_text(&mut final_svg, container_width, container_height, &by_square_text_color);
                    insert_pay_text(&mut final_svg, container_width, container_height, &pay_text_color);

                    let mut final_qr = Vec::new();
                    let emitter_options = EmitterConfig::default()
                        .write_document_declaration(false);
                    final_svg.write_with_config(&mut final_qr, emitter_options).expect("unable to write path");
                    println!("FINAL SVG CONTENT: {}", String::from_utf8(final_qr.clone()).unwrap());

                    // content_to_show = Some(svg_image);
                    content_to_show = Some(String::from_utf8(final_qr).unwrap());
    ...
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default().with_inner_size([512.0, 600.0]),
        ..Default::default()
    };

    match content_to_show {
        Some(svg_data) => {
            eframe::run_simple_native("Pay By Square Preview", options, move |ctx, _frame| {
                ...
                egui::CentralPanel::default().frame(bg).show(ctx, |ui| {
                    let svg_tree = Tree::from_str(&svg_data, &Options::default()).unwrap();
                    let svg_image = Some(RetainedImage::from_svg_bytes("example.svg", svg_tree.to_string(&WriteOptions::default()).as_bytes()).unwrap());

                    svg_image.unwrap().show(ui)
                });
            }).expect("");
        }
        None => {}
